...
export function isValidCartItem(item) {
  ...
  const title = String(item.title || item.product_title || '').trim();
...
export function mergeCartItems(remoteItems = [], incomingItems = [], options = {}) {
  ...
  // 1. Index valid remote items currently in Supabase (discarding any existing corrupt rows)
  if (Array.isArray(remoteItems)) {
    for (const item of remoteItems) {
      ...
      // Discard corrupt rows from remote DB
      if (!isValidCartItem(item)) {
        console.warn(`[CartEngine][WARN] Discarding corrupt remote row for customer (variant_id: "${variantId}")`);
        continue;
      }

      // If user performed an explicit mutation on client, any remote variant omitted from client payload was deleted
      if (isExplicitMutation && !isGuestMigration && !incomingVariantIds.has(variantId)) {
        continue;
      }
  ...
  if (Array.isArray(incomingItems)) {
    for (const item of incomingItems) {
      ...
      const productId = String(item.product_id || existing?.product_id || '').trim();
      const title = String(item.product_title || item.title || existing?.title || '').trim();
      ...
      // Strict validation: Skip inserting incomplete/corrupt incoming items that lack productId or title
      if (!isValidCartItem(sanitizedIncoming)) {
        console.warn(`[CartEngine][WARN] Skipping corrupt incoming item (variant_id: "${variantId}", product_id: "${productId}", title: "${title}")`);
        continue;
      }
